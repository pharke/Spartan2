...
fn main() {
  ...
  for circuit in circuits {
    ...
    let t0 = Instant::now();
    let prep_snark =
      SpartanZkSNARK::<E>::prep_prove(&pk, circuit.clone(), true).expect("prep_prove failed");
    ...
    let t0 = Instant::now();
    let proof =
      SpartanZkSNARK::<E>::prove(&pk, circuit.clone(), &prep_snark, true).expect("prove failed");
