...
use crate::{
  ...
  pub struct SpartanPrepZkSNARK<E: Engine> {
    ...
    precommit_challenges: Vec<E::Scalar>,
    override_challenges: Vec<E::Scalar>,
  }

  impl<E: Engine> SpartanPrepZkSNARK<E> {
    /// Returns the intermediate challenges derived during precommit.
    pub fn precommit_challenges(&self) -> &[E::Scalar] {
      &self.precommit_challenges
    }

    /// Overrides the challenges used during proving with external challenges.
    pub fn set_external_challenges(
      &mut self,
      external_challenges: Vec<E::Scalar>,
    ) -> Result<(), SpartanError> {
      if external_challenges.len() != self.precommit_challenges.len() {
        return Err(SpartanError::InvalidInputLength {
          reason: format!(
            "External challenges length mismatch: expected {}, got {}",
            self.precommit_challenges.len(),
            external_challenges.len()
          ),
        });
      }
      self.override_challenges = external_challenges;
      Ok(())
    }
  }
  ...
  /// Extra verifier outputs used by external challenge-derivation protocols.
  #[derive(Clone, Debug)]
  pub struct VerifyArtifacts<E: Engine> {
    /// Intermediate challenges derived only from this proof's precommitments.
    pub intermediate_challenges: Vec<E::Scalar>,
  }

impl<E: Engine> SpartanZkSNARK<E> {
  /// Pre-commits shared and precommitted witness parts.
  pub fn precommit<C: SpartanCircuit<E>>(
    ...
    let precommit_challenges =
      SatisfyingAssignment::derive_precommitted_challenges(&ps, &pk.S, &mut transcript)?;
    Ok(SpartanPrepZkSNARK {
      ps,
      precommit_challenges: precommit_challenges.clone(),
      override_challenges: precommit_challenges,
    })
  }

  /// Verifies the proof and returns intermediate challenges for external checking.
  pub fn verify_with_artifacts(
    &self,
    vk: &SpartanVerifierKey<E>,
  ) -> Result<VerifyArtifacts<E>, SpartanError>
  where
    E::PCS: FoldingEngineTrait<E>,
  {
    ...
    // Derive intermediate challenges on an independent transcript so we do not
    // perturb the main verify transcript used for proof verification.
    let mut transcript_intermediate = E::TE::new(b"SpartanZkSNARK");
    transcript_intermediate.absorb(b"vk", &vk_digest);
    let intermediate_challenges =
      self.U.validate_and_derive_intermediate(&vk.S, &mut transcript_intermediate)?;

    let mut transcript = E::TE::new(b"SpartanZkSNARK");
    transcript.absorb(b"vk", &vk_digest);
    self
      .U
      .validate_external_challenge_instance(&vk.S, &mut transcript)?;
    ...
    E::PCS: FoldingEngineTrait<E>,
  {
    ...
    Self::precommit(pk, circuit, is_small)
    }
    ...
    fn prove<C: SpartanCircuit<E>>(
      ...
      let mut ps = prep_snark.ps.clone();
      ...
      let (U, W) = SatisfyingAssignment::r1cs_instance_and_witness_with_external_challenges(
        &mut ps,
        &pk.S,
        &pk.ck,
        &circuit,
        is_small,
        &mut transcript,
        &prep_snark.override_challenges,
      )?;
    ...
    fn verify(&self, vk: &Self::VerifierKey) -> Result<Vec<E::Scalar>, SpartanError> {
      let _ = self.verify_with_artifacts(vk)?;
  ...
  mod tests {
    ...
    #[derive(Clone, Debug, Default)]
    struct OneChallengeCircuit {}

    impl<E: Engine> SpartanCircuit<E> for OneChallengeCircuit {
      fn public_values(&self) -> Result<Vec<<E as Engine>::Scalar>, SynthesisError> {
        Ok(vec![E::Scalar::from(9u64)])
      }
      ...
      fn synthesize<CS: ConstraintSystem<E::Scalar>>(
        ...
        let challenge_value = challenges
          .and_then(|chals| chals.first().copied())
          .unwrap_or(E::Scalar::ZERO);
        let chal = AllocatedNum::alloc(cs.namespace(|| "challenge"), || Ok(challenge_value))?;
        let x = AllocatedNum::alloc(cs.namespace(|| "x"), || Ok(E::Scalar::from(2u64)))?;
        let z = AllocatedNum::alloc(cs.namespace(|| "z_alloc"), || {
          Ok(x.get_value().unwrap() + chal.get_value().unwrap())
        })?;

        cs.enforce(
          || "z_constraint",
          |lc| lc + x.get_variable() + chal.get_variable(),
          |lc| lc + CS::one(),
          |lc| lc + z.get_variable(),
        );

        let p = AllocatedNum::alloc(cs.namespace(|| "public_value"), || Ok(E::Scalar::from(9u64)))?;
        let _ = p.inputize(cs.namespace(|| "public_value_input"));
        let _ = chal.inputize(cs.namespace(|| "challenge_input"));
        Ok(())
      }
    }

    #[test]
    fn test_zksnark_with_external_challenge_override() {
      type E = crate::provider::PallasHyraxEngine;
      type S = SpartanZkSNARK<E>;
      ...
      let external_challenges = prep_snark.precommit_challenges().to_vec();
      prep_snark
        .set_external_challenges(external_challenges)
        .unwrap();

      let snark = S::prove(&pk, circuit.clone(), &prep_snark, false).unwrap();
      let res = snark.verify(&vk);
      assert!(res.is_ok());
      assert_eq!(res.unwrap(), [<E as Engine>::Scalar::from(15u64)]);

      let artifacts = snark.verify_with_artifacts(&vk).unwrap();
      assert_eq!(
        artifacts.intermediate_challenges.len(),
        pk.sizes()[9], // num_challenges
      );
      assert_eq!(
        artifacts.intermediate_challenges,
        prep_snark.precommit_challenges().to_vec()
      );
    }

    #[test]
    fn test_zksnark_external_challenge_override_nonzero_challenges() {
      type E = crate::provider::PallasHyraxEngine;
      type S = SpartanZkSNARK<E>;

      let circuit = OneChallengeCircuit::default();
      let (pk, vk) = S::setup(circuit.clone()).unwrap();
      let mut prep_snark = S::prep_prove(&pk, circuit.clone(), false).unwrap();

      let intermediate = prep_snark.precommit_challenges().to_vec();
      assert_eq!(intermediate.len(), 1);

      let external = vec![intermediate[0] + <E as Engine>::Scalar::ONE];
      prep_snark.set_external_challenges(external.clone()).unwrap();

      let snark = S::prove(&pk, circuit, &prep_snark, false).unwrap();
      assert_eq!(snark.U.challenges, external);

      let res = snark.verify(&vk);
      assert!(res.is_ok());
      assert_eq!(res.unwrap(), vec![<E as Engine>::Scalar::from(9u64)]);

      let artifacts = snark.verify_with_artifacts(&vk).unwrap();
      assert_eq!(artifacts.intermediate_challenges, intermediate);
    }
  }
