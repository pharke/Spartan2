...
impl<Scalar: PrimeField> BigNat<Scalar> {
  /// Returns the limb width in bits for this `BigNat`.
  pub fn limb_width(&self) -> usize {
    self.params.limb_width
  }

  /// Returns the number of limbs used to represent this `BigNat`.
  ...
  /// Convert this `BigNat` to a vector of `Num` limbs.
  ...
  /// Assert that all limbs of this `BigNat` are well-formed (fit within their bit width).
  ...
  /// Enforce that this `BigNat` and `other` have the same limb width.
  ...
  /// Create a `BigNat` from a polynomial representation.
  ...
  /// Add two `BigNat` values together.
  ...
  /// Return the number of bits needed to represent this `BigNat`.
...
// NOTE: Original tests depended on Nova's `frontend` and are removed during migration to Spartan2.
