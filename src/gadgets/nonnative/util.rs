...
/// A representation of a number in the constraint system.
...
impl<Scalar: PrimeField> Num<Scalar> {
  /// Create a new `Num` from a value and a linear combination.
  pub const fn new(value: Option<Scalar>, num: LinearCombination<Scalar>) -> Self {
    Self { value, num }
  }
  /// Allocate a `Num` in the constraint system.
  ...
  /// Check that this number fits within a given number of bits.
  ...
  /// Convert this `Num` to an `AllocatedNum`.
